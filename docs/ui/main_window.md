...
| `_select_file()` | ファイル選択ダイアログ表示 |
| `_run_guitar()` | Guitar TAB 解析を `_launch` 経由で起動 |
| `_run_bass()` | Bass TAB 解析を `_launch` 経由で起動 |
| `_run_drum()` | Drum 解析を `_launch` 経由で起動 |
...
| `_set_analysis_enabled(bool)` | 解析ボタン群の有効/無効を一括切替 |
